# Tên chương trình: floating point division in MARS_MIPS
.data
	#bien doc file
	...
	tab_string:	.asciiz "\t"
	fp0:		.float 0.0
	fp_a:		.float	10.23456		#so bi chia
	fp_b:		.float	5.2345678	#so chia
	fp_res:		.float  1.0
...
div_fp:
	...
	ori 	$t6, $t6, 0x00800000	#add 1 to fraction ~= (1 + fraction)
	ori	$t7, $t7, 0x00800000 	#add 1 to fraction ~= (1 + fraction)
	#check denormalize: if exponent = 0 -> denormalize -> not add 1 to fraction
	bnez	$t4, notplus1
		addi	$t4, $t4, 1
		andi	$t6, $t6, 0x007fffff #not add 1 to fraction
	notplus1:
	bnez	$t5, notplus2
		addi	$t5, $t5, 1
		andi	$t7, $t7, 0x007fffff #not add 1 to fraction
	notplus2:
...
get_sign:
	...
	#a0 32 bit floating point number 
	#return the sign of a0
